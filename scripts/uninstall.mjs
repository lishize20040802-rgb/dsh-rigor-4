...
import { buildPlan, parseArgs, runCommand } from './install.mjs'
...
export function previewUninstall(plan) {
  return { mode: 'dry-run', package: 'dsh-rigor-4', profile: plan.profile,
    command: ['dsh', 'plugin', '--profile', plan.profile, 'remove', 'dsh-rigor-4', '--offline', '--ignore-scripts', '--ignore-pnpmfile'],
...
export async function applyUninstallPlan(plan, { run = runCommand } = {}) {
  ...
  if (plan.installed) await run({ label: 'dsh plugin remove', executable: plan.dsh.executable,
    args: [...plan.dsh.prefix, 'plugin', '--profile', plan.profile, 'remove', 'dsh-rigor-4', '--offline', '--ignore-scripts', '--ignore-pnpmfile'],
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    if (options.help) console.log('node scripts/uninstall.mjs [--dry-run|--apply] [--dsh-home PATH] [--profile NAME] [--dsh-package PATH]')
